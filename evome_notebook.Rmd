...
Based on the above plot, it appears that most of the missing values are from low abundant proteins. Here, we will use the QRILIC method of data imputation.
...
DEP log2 transforms the values and then imputes. We will use this matrix as input into limma for comparing protein abundances between the Heavy and Light fractions of EVs. We note that one check that the data fits limma's assumptions is that variance is a monotonically decreasing function of average expression. We can check this assumption. 
...
volcanoplot(fit.ev.w.weights,highlight = nrow(diff.exp.top.w.weights),names=rownames(diff.exp.top.w.weights))
```

Based on the limma analysis, we detect 295 genes with an absolute log2 fold change of at least 1.5 and adjusted p-value < 0.05 out of the 1016 proteins. Now let's perform a KEGG enrichment of the genes which appear differentially between the heavy and light strains. 
...
diff.exp.kegg.ids <- gene.to.kegg[diff.exp.top.w.weights$Row.names,1]
diff.exp.kegg.ids <- c(diff.exp.kegg.ids,only.heavy,only.light)
gene.universe <- gene.to.kegg[rownames(caeel.unique.sc),1]
...
diff.exp.kegg.ids <- gene.to.kegg[diff.exp.top.w.weights[which(diff.exp.top.w.weights$logFC > 0),"Row.names"],1]
only.heavy.kegg <- gene.to.kegg[only.heavy,"KEGG_Gene_ID"]
heavy.kegg <- c(only.heavy.kegg,diff.exp.kegg.ids)
#heavy.kegg <- c(diff.exp.kegg.ids)
...
write.table(light.enrichment@result,"Results/light_fraction_upregulated_kegg_enrichment_20210901.tsv",sep="\t",col.names=T,row.names=F,quote=F)
...
write.table(all.enrichment@result,"Results/evome_vs_background_kegg_enrichment_20210901.tsv",sep="\t",col.names=T,row.names=F,quote=F)
